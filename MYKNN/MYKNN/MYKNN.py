...
def kNNClassify(newInput, dataSet, labels, k, i):
...
#    for k in range(key_num):
    for i in range(test_num):
            ...
            predict = kNNClassify(test_x[i], train_x, train_y, 3, i)
            if predict == test_y[i]:
                matchCount += 1
            if i % 100 == 0:
                print ("已分析%d张图片"%(i))
    ...
    print ('step 4:When k=%d, the classify accuracy is: %.2f%%' % (k,accuracy * 100))
